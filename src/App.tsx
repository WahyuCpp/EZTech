...
export default function EZTechPalembang() {
  ...
  const renderEmployeePortal = () => {
    ...
    const todayAttendance = attendances.find(
      (att: Attendance) => new Date(att.date).toDateString() === new Date().toDateString()
    );
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg font-bold text-foreground">
                    {
                      attendances.filter(
                        (att: Attendance) => new Date(att.date).getMonth() === new Date().getMonth()
                      ).length
                    }
                  </span>
                </div>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <Card>
            ...
            <CardContent>
                <p className="text-4xl font-bold text-foreground">
                  {services.filter((s: Service) => s.status === 'pending').length}
                </p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
                <p className="text-4xl font-bold text-foreground">
                  {services.filter((s: Service) => s.status === 'completed').length}
                </p>
            </CardContent>
          </Card>
        </div>
